...
<div class="row-fluid">
	<div class="span12 portlet box blue">
		<div class="portlet-title">
			<div class="caption">
				<i class="icon-globe"></i><span class="titleExcel">库存列表</span>
			</div>
		</div>
			...
			<table id="tbList" class="table table-striped table-bordered table-hover table-full-width" data-click-to-select="true" data-single-select="true">
				<thead>
					<tr>
						...
						<th data-field="STOCKCNT" data-formatter="stockCntFormatter">库存</th>
...
function doSearch()
{
	...
	var url = "report/getStockList.do?fmtm=" + fmtm +"&totm=" + totm + "&dccd=" + dccd + "&mtrlcd=" + $("#orderMaterials").val();
...
function stockCntFormatter(value, row, index)
{
	var stockCnt = row.STOCKCNT == undefined ? 0 : row.STOCKCNT;
	return '<span>' + currency(stockCnt) + '</span>';
}
...
function actlAmntFormatter(value, row, index)
{
	...
	var stockCnt = row.STOCKCNT == undefined ? 0 : row.STOCKCNT;
	return '<span class="actlAmnt">' + currency(ptMul(actlprc, stockCnt)) + '</span>';
}
...
function saleAmntFormatter(value, row, index)
{
	...
	var stockCnt = row.STOCKCNT == undefined ? 0 : row.STOCKCNT;
	return '<span class="saleAmnt">' + currency(ptMul(saleprc, stockCnt)) + '</span>';
}
