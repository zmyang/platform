...
<div class="row-fluid">
	<div class="span12 portlet box blue">
		<div class="portlet-title">
			<div class="caption">
				<i class="icon-globe"></i><span class="titleExcel">盘点差异报表</span>
			</div>
		</div>
			...
			<table id="tbList" class="table table-striped table-bordered table-hover table-full-width" data-click-to-select="true" data-single-select="true">
				<thead>
					<tr>
						...
						<th data-formatter="bizdtFormatter">日期</th>
						<th data-field="DFCNT" data-formatter="dfCntFormatter">差异数量</th>
...
function doSearch()
{
	...
	var fmtm = times[0];
	var totm = times[2];
	var url = "report/geChkDfList.do?fmtm=" + fmtm + "&totm=" + totm + "&dccd=" + dccd;
...
function dfCntFormatter(value, row, index)
{
	var dfCnt = row.DFCNT == undefined ? 0 : row.DFCNT;
	return '<span>' + currency(dfCnt) + '</span>';
}
...
function moneyFormatter(value, row, index)
{
	...
	var dfCnt = row.DFCNT == undefined ? 0 : row.DFCNT;
	return '<span class="money">' + currency(ptMul(saleprc, dfCnt)) + '</span>';
}

function bizdtFormatter(value, row, index)
{
	var bizdt = row.BIZDT.substring(5);
	return '<span>' + bizdt + '</span>';
}
